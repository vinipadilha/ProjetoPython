...
def turmas(request):
    ...
    turmas = Turma.objects.filter(matriculas__aluno_id=aluno_id).select_related('treinamento').order_by('-tru_data_inicio')
    return render(request, 'aluno/listar.html', {'turmas': turmas, 'aluno': aluno})
